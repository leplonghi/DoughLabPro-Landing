...
const ProToolsSection = () => {
  const { t } = useLanguage();
  const toolsString = t('proTools_tools');
  let tools: string[] = [];
  if (toolsString && toolsString.startsWith('[')) {
    try {
        tools = JSON.parse(toolsString);
    } catch(e) {
        console.error("Failed to parse pro tools", e);
    }
  }

  return (
    <section id="pro-tools" className="py-20 sm:py-28 bg-secondary/50">
        ...
        <div className="mt-12 max-w-3xl mx-auto">
            <ul className="space-y-4">
                {tools.map((tool: string) => (
                    <li key={tool} className="flex items-start text-left">
                        <CheckIcon />
